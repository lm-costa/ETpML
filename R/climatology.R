...
### AF
file_names <- list.files('data-raw/bh_af/',full.names = T)
...
df_climatology_resume <- df_climatology |>
...
df_climatology_resume <- df_climatology_resume |>
  dplyr::mutate(
    nda= dplyr::case_when(
      month==1~1,
      month==2~31,
...
for(i in 1:nrow(df_climatology_resume)){
  month <- df_climatology_resume$month
  prec <- df_climatology_resume$prec
  ETP <- df_climatology_resume$ETP
  if(month[i]==1){
    petp <- prec-ETP
    if(petp[i]<0){
      nac[i] <- petp[i]
...
tab_f <- data.frame(
  station=paste0(df_climatology_resume$longitude,'_',df_climatology_resume$latitude),
  month=df_climatology_resume$month,
  prec=df_climatology_resume$prec,
  temp=df_climatology_resume$temp,
  etp= df_climatology_resume$ETP,
  etr=etr,
  def=def,
  exc=sur
...
write.csv(tab_f |>
            ...
            ,'data/bh_af/climatology.csv')




### Cerr
file_names <- list.files('data-raw/bh_cerr/',full.names = T)
...
write.csv(tab_f |>
          ...
          ,'data/bh_cerr/climatology.csv')




####### CAAT
file_names <- list.files('data-raw/bh_caat/',full.names = T)
...
write.csv(tab_f |>
          ...
          ,'data/bh_caat/climatology.csv')



####### AMZ
file_names <- list.files('data-raw/bh_amz/',full.names = T)
